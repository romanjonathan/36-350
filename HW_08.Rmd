...
tester <- function(func) {
  test_that(desc = "Error: func(\"a\") should == 0", expect_equal(func("a"), 0))
  test_that(desc = "Error: func(\"A\") should == 0", expect_equal(func("A"), 0))
  test_that(desc = "Error: func(1) should have thrown a non char input exception",
         expect_error(func(1)))
  test_that(desc = "Error: func(list of strings) should throw an exception",
         expect_error(func(c("b", "b", "c"))))
  test_that(desc = "Error: no warning issued when using only the first char of a
...
myF = function(letter) {
  ...
  if(length(letter) != 1) stop("The input must not be a list of strings")
