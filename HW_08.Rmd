...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/romanjonathan/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/romanjonathan/36-350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/romanjonathan/36-350/main/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/romanjonathan/36-350/new_branch/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/romanjonathan/36-350-Fork/main/check_mark.R")
```
---
...
tester <- function(func) {
  test_that(desc = "Error: test that lower-case letters work;
            func(\"a\") should == 0",
            expect_equal(func("a"), 0))
  test_that(desc = "Error: test that upper-case letters work;
            func(\"A\") should == 0",
            expect_equal(func("A"), 0))
  test_that(desc = "Error: test for improper input (numeric);
            func(1) should have thrown a non char input exception",
            expect_error(func(1)))
  test_that(desc = "Error: test that the length of the input vector is 1;
            func(list of strings) should throw an exception",
            expect_error(func(c("b", "b", "c"))))
  test_that(desc = "Error: test for string of length greater than 1;
            no warning issued when using only the first char of a string of
            length greater than 1",
            expect_warning(func("yo")))
}
...
myF = function(letter) {
  ...
  if(length(letter) != 1) stop("The input must a single string, not a list")
  if (nchar(letter) > 1) warning("The input character has length >1,
                                      using only the first character")
  return(log(which(letters==tolower(letter[1]))))
}
tester(myF)
```
